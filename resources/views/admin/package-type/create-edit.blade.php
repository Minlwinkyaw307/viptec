...
@section('content')
    <div class="table-list mt-6 w-full">
        ...
        <div class="w-full flex items-center justify-between mx-auto">
            <div class="w-1/6"></div>
            <div class="w-4/6">
                <form action="{{ isset($data) ? route('admin.package-type.update', ['package_type' => $data->id]) : route('admin.package-type.store') }}" method="post">
                    @csrf
                    @isset($data)
                        @method('put')
                    @endisset
                    <x-admin::text-input
                        ...
                        :placeholder="__('Please Enter Package Type Name')"
