            ...
            <div class="table-list-header-input">
                <div class="form-group">
                    <div class="form-element">
                        <button class="button-first focus:outline-none link-button" data-link="{{ route('admin.certificate.create') }}">{{ __("Create New Item") }}</button>
                    </div>
                </div>
            </div>
        </div>
                ...
                <form action="">
                    ...
                    <td class="filter">
                        <x-admin::text-input
                            name="filter_title"
                            :label="null"
                            :value="request()->query('filter_title')"
                            id="filter_name"
                            :placeholder="__('Please Enter Certificate Name')"
                            :required="false"
                        ></x-admin::text-input>
                    </td>
                    <td class="filter">
                    ...
                    <td class="filter">
                        <x-admin::select-input
                            ...
                            value="1"
                            :options="\App\Models\Category::category_statues()"
                            :label="null"
                ...
                <tr>
                    ...
                    <td>
                        <div class="product-box flex items-center">
                            <div class="product-box-image"><a class="block" href="javascript:void(0);"><img src="{{ $row->imageUrl }}" alt=""></a></div>
                            <div class="product-box-content"><span class="block">{{ $row->translations[0]->title }}</span>
                            </div>
                        </div>
                    </td>
                    ...
                    <td>
                        <div class="process-buttons flex">
                            <button  class="focus:outline-none link-button" data-link="{{ route('admin.certificate.edit', ['certificate' => $row->id]) }}">
                                <svg viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                    <path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path>
                                </svg>
                            </button>
                            <form action="{{ route('admin.certificate.destroy', ['certificate' => $row->id]) }}" method="post" onsubmit="return confirm('{{__("Are your sure about deleting selected item?")}}')">
                                @csrf
                                @method('delete')
