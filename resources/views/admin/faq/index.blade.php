            ...
            <div class="table-list-header-input">
                <div class="form-group">
                    <div class="form-element">
                        <button class="button-first focus:outline-none link-button" data-link="{{ route('admin.faq.create') }}">{{ __("Create New Item") }}</button>
                    </div>
                </div>
            </div>
        </div>
                ...
                <form action="">
                    ...
                    <td class="filter">
                        <x-admin::text-input
                            ...
                            :label="null"
                            :value="request()->query('filter_question')"
                            id="filter_question"
                    ...
                    <td class="filter">
                        <x-admin::text-input
                            name="filter_answer"
                            :label="null"
                            :value="request()->query('filter_answer')"
                            id="filter_answer"
                            :placeholder="__('Please Enter Question')"
                            :required="false"
                        ></x-admin::text-input>
                ...
                <tr>
                    ...
                    <td>
                        <div class="product-box flex items-center">
                            <div class="product-box-content"><span class="block">{{ $row->translations[0]->question }}</span>
                            </div>
                        </div>
                    </td>
                    <td>
                        <div class="product-box flex items-center">
                            <div class="product-box-content"><span class="block">{{ \Illuminate\Support\Str::limit($row->translations[0]->answer, 25) }}</span>
                            </div>
                        </div>
                    </td>
                    ...
                    <td>
                        <div class="process-buttons flex">
                            <button  class="focus:outline-none link-button" data-link="{{ route('admin.faq.edit', ['faq' => $row->id]) }}">
                                <svg viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                    <path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path>
                                </svg>
                            </button>
                            <form action="{{ route('admin.faq.destroy', ['faq' => $row->id]) }}" method="post" onsubmit="return confirm('{{__("Are your sure about deleting selected item?")}}')">
                                @csrf
                                @method('delete')
